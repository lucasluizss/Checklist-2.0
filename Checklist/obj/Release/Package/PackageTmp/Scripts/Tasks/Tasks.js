    ...
    if (task.IdStep != 0) {
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#' + idUpgrade + ' .ProgressStep').html("⌚️ " + data);
...
function getEditTask(id) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('.ed-modal-lg').modal();
            $('.date').mask('00/00/0000');
            $(GeneralSettings.TitleModal).val(data.Title);
            $(GeneralSettings.DescriptionModal).val(data.Description);
            $(GeneralSettings.PriorityModal).val(data.IdPriority);
            $(GeneralSettings.StartDateModal).val(moment(data.StartDate).format('L'));
            $(GeneralSettings.EndDateModal).val(moment(data.EndDate).format('L'));
            idUpgrade = data.IdTask;
        },
        error: function (error) {
            console.log(error);
